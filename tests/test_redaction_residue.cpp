...
struct ResidueFixture {
    ...
    explicit ResidueFixture(const std::string& db_path = "", bool seed = true) {
        ...
        // security/push made the gateway allowlist DEFAULT-CLOSED: an empty
        // allowed_gateway_prefixes now means "no gateway may be registered",
        // so /pushers/set 400s and nothing reaches the queue. These tests
        // predate that and were asserting on an empty queue for the wrong
        // reason — their own probe says "nothing was queued; the probe proves
        // nothing". Allow exactly the gateway this fixture uses.
