...
TEST(AuditMigration, FreshDatabaseGetsV13WithAnAppendOnlySchema) {
    ...
    // v13 is the audit log; the pin tracks the latest schema, now 20 (v14
    ...
    // backfill onto pre-existing DMs' membership state, v19 push_queue.event_id
    // so a redaction reaches an undispatched notification, v20 the repair of
    // redactions that left a message's edits readable). This test's subject
