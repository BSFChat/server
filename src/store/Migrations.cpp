...
// v19: `push_queue.event_id` — a queued notification remembers which event it
// is about.
//
...
void migrate_v19(sqlite3* db, bool /*fresh_database*/) {
    ...
    if (orphaned > 0) {
        get_logger()->info(
            "Schema v19: dropped {} queued push notification(s) predating event-id tracking; "
            "they could not be matched against a redaction, so they are not safe to deliver.",
            orphaned);
    }
}

// v20: finish the redactions this server has already performed.
//
...
void migrate_v20(sqlite3* db, bool fresh_database) {
    ...
    if (repaired > 0) {
        get_logger()->warn(
            "Schema v20: stripped {} message edit(s) left readable by an earlier redaction. "
