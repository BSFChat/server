...
int SqliteStore::delete_pushers_by_pushkey_except(const std::string& pushkey,
    ...
    // Same rule as delete_pusher(): a pushkey that has moved to another account
    // must not keep delivering the previous account's queued messages.
    auto del = prepare(db_,
        "DELETE FROM push_queue WHERE pushkey = ? AND NOT (user_id = ? AND app_id = ?)");
    sqlite3_bind_text(del.get(), 1, pushkey.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(del.get(), 2, keep_user_id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(del.get(), 3, keep_app_id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(del.get());
    return removed;
}
